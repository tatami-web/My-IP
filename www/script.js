document.addEventListener('DOMContentLoaded', function () {
    ...
    let lastFetchTime = 0;
    let refreshInterval = null; // Almacena el intervalo del contador
    ...
    // Contador en el botón de refrescar
    function startRefreshCounter() {
        let counter = 0; // Inicializa el contador
        refreshBtn.disabled = true; // Desactiva el botón mientras cuenta
        refreshBtn.textContent = counter; // Muestra el contador

        refreshInterval = setInterval(() => {
            counter++; // Incrementa el contador
            refreshBtn.textContent = counter; // Actualiza el texto del botón

            if (counter >= 5) {
                clearInterval(refreshInterval); // Detiene el intervalo al llegar a 5
                refreshBtn.textContent = 'Refresh'; // Restaura el texto del botón
                refreshBtn.disabled = false; // Reactiva el botón
            }
    ...
    refreshBtn.addEventListener('click', function () {
        if (!refreshBtn.disabled) {
            startRefreshCounter(); // Inicia el contador
            fetchIP(); // Llama a la función de refrescar
        }
    });
